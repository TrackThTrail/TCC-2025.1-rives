...
class SnakeGame:
    ...
    def get_state(self):
        head = self.snake[0]
        point_l = self.move_point(head, (self.direction + 3) % 4)  # esquerda
        point_r = self.move_point(head, (self.direction + 1) % 4)  # direita
        point_f = self.move_point(head, self.direction)           # frente

        danger_left = self.is_collision(point_l)
        danger_right = self.is_collision(point_r)
        danger_straight = self.is_collision(point_f)
...
def train():
    game = SnakeGame()
    agent = DQNAgent()
    scores = []       # guarda maçãs comidas por episódio
    avg_scores = []   # médias a cada 100 episódios
    episodes_avg = [] # episódios para o eixo x
    max_apples = 0
    ...
    for episode in range(1, 20001):
        ...
        # maçãs comidas no episódio = tamanho da cobra - 1
        apples_eaten = len(game.snake) - 1
        scores.append(apples_eaten)

        if apples_eaten > max_apples:
            print(f"New Max Apples! {apples_eaten}")
            max_apples = apples_eaten

        if episode % 100 == 0:
            avg_apples = sum(scores[-100:]) / 100
            avg_scores.append(avg_apples)
            episodes_avg.append(episode)
            print(f"Episodes {episode-99} to {episode} — Average Apples: {avg_apples:.2f}, Epsilon: {agent.epsilon:.3f}")

    # Plotar gráfico da média de maçãs comidas
    plt.plot(episodes_avg, avg_scores, marker='o')
    plt.xlabel("Episode")
    plt.ylabel("Average Apples Eaten (per 100 episodes)")
    plt.title("DQN Snake - Average Apples")
    plt.grid()
    plt.show()
